...
def p_inicio(p):
	'''INICIO :  TkWith DEC TkBegin INSTR TkEnd
			| TkBegin INSTR TkEnd'''
	if (len(p) == 6):
		p[0] = InstrTree("Inicio con declaraciones", [p[2], p[4]], [p[1], p[3], p[5]])
	else:
		p[0] = InstrTree("Inicio", [p[2]], [p[1], p[3]]) 
...
def p_dec(p):
	...
	if (len(p) == 6):
		p[0] = InstrTree("Lista de Varias Declaraciones", [p[1], p[3], p[5]], [p[2], p[4]])
	else:
		p[0] = InstrTree("Lista de Declaraciones", [p[2], p[4]], [p[1], p[3]])
...
def p_tipo(p):
	...
	if (len(p) == 2):
		...
		p[0] = InstrTree("Tipo Arreglo", [p[3], p[6]], [p[1], p[2], p[4], p[6]])
...
def p_asig(p):
	...
	if (len(p) == 4):
		p[0] = InstrTree("Asignacion", [p[3]], [p[1], p[2]])
	else:
		p[0] = InstrTree("Declaracion de Asignacion", [p[3]], [p[1], p[2]])
...
def p_arit(p):
	...
	if (len(p) == 2):
		...
		if ((p[1] == '(') and (p[3] == ')')):
			p[0] = InstrTree("Expresion Aritmetica", [p[2]], [p[1], p[3]])
		else:
			p[0] = InstrTree("Expresion Aritmetica", [p[1], p[3]], [p[2]])
	else:
		p[0] = InstrTree("Expresion Aritmetica", None, [p[2], p[3]])
...
def p_bool(p):
	...
	if (len(p) == 4):
		if ((p[1] == '(') and (p[3] == ')')):
			p[0] = InstrTree("Expresion Booleana", p[2], [p[1], p[3]])
		else:
...
def p_char(p):
	...
	if (len(p) == 2):
		...
		p[0] = InstrTree("Expresion con Caracteres", None, [p[2], p[1]])
...
def p_cond(p):
	...
	p[0] = InstrTree("Condicional General", [p[2], p[4], p[5]], [p[1], p[3]])
...
def p_cond0(p):
	'''COND0 :    TkOtherwise TkHacer INSTR
				| TkEnd'''
	if ( len(p) == 4):
		p[0] = InstrTree("Condicional", [p[3]], [p[1], p[2]])
	else:
...
def p_indeter(p):
	...
	p[0] = InstrTree("Iteracion Indeterminada", [p[2], p[4]], [p[1], p[3], p[5]])

# Iteracion determinada general

def p_deter(p):
	'''DETER :   TkFor TkId TkFrom ARIT TkTo ARIT DETER0'''
	p[0] = InstrTree("Iteracion Determinada General", [p[4], p[6], p[7]], [p[1], p[2], p[3], p[5]])
...
def p_deter0(p):
	'''DETER0 :  TkStep ARIT TkHacer INSTR TkEnd
			| TkHacer INSTR TkEnd'''
	if (len(p) == 6):
		p[0] = InstrTree("Iteracion Determinada Step", [p[2], p[4]], [p[1], p[3], p[5]])
	else:
		p[0] = InstrTree("Iteracion Determinada",[ p[2]], [p[1], p[3]])
...
def p_i_o(p):
	...
	p[0] = InstrTree("Entrada o Salida", None, [p[1], p[2]])
...
def p_expr(p):
	...
	if (len(p) == 2):
		...
		p[0] = InstrTree("Indexacion", [p[3]], [p[1], p[2], p[4]])
